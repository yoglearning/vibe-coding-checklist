const checklistData = [
  ...
  { phase: "Testing & Quality Assurance", items: [
    ...
    { type: "Generic", text: "Run Lighthouse audits and address any scores below 90." },
      { type: "Generic", text: "Ask the AI: \"What security risks could leak data, expose secrets, or weaken access control in this code?\"" },
      { type: "Generic", text: "Ensure no secrets, tokens, API keys, or credentials are hardcoded, committed, or exposed client-side; verify they are loaded securely from environment configuration." },
      { type: "Generic", text: "Validate, sanitize, and encode all user inputs on both client and server; double-check paths that touch auth, file upload, queries, or rendered content." },
      { type: "Generic", text: "Ask the AI to add rate limiting, abuse protection, or CAPTCHA where appropriate, and verify the controls cannot be bypassed easily." },
  ...
  { phase: "Polish, Security & Handover", items: [
    ...
    { type: "Generic", text: "Ask the AI: \"List all WCAG 2.2 rules this app might violate.\"" },
      { type: "Generic", text: "Fix or explicitly review every dependency scanner warning (e.g., npm audit), and confirm no unresolved issue can expose data in production." },
      { type: "Generic", text: "Ensure all secrets are rotated before final launch, least-privilege access is enforced, and exposed test credentials are fully revoked." },
